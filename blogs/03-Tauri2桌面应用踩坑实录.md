...
> 用 Tauri 2 做了一个 Windows 本地优先的题库应用，最终安装包 9.5MB（含题库与 962 个文档的检索语料）。真正花时间的不是"能不能跑起来"，而是五件事：WebView2 分发矛盾、FTS5 中文分词默认不可用、本地图片的自定义协议、WAL 模式备份一致性、SM-2 状态机的零间隔死锁。每件事都有可直接抄的解法。
